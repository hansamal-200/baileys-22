...
// ════════════════════════════════════════════════════════════════════════════
// useMultiFileAuthState — file-based auth state, one file per key, atomic writes
// ════════════════════════════════════════════════════════════════════════════
...
// Module-level cache: same connection string always reuses the same underlying adapter
// instance/connection pool across every session and reconnect in the whole process.
    ...
    const keys = {
        get: async (type, ids) => {
            ...
            await Promise.all(ids.map(async (id) => {
                ...
                data[id] = patchAppStateKey(type, deser(await keyv.get(key), logger, (fixed) => keyv.set(key, fixed)))
            }))
            return data
        },
        set: async (data) => {
