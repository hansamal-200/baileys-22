...
// ════════════════════════════════════════════════════════════════════════════
const ser = (v) => { if (v == null) return null; const serialized = JSON.stringify(v, BufferJSON.replacer); return { data: JSON.parse(serialized), __checksum: computeChecksum(serialized) } }
const deser = (wrapped) => { if (wrapped == null) return null; if (typeof wrapped === 'object' && 'data' in wrapped && '__checksum' in wrapped) { const expected = computeChecksum(JSON.stringify(wrapped.data)); return expected === wrapped.__checksum ? JSON.parse(JSON.stringify(wrapped.data), BufferJSON.reviver) : null }; return JSON.parse(JSON.stringify(wrapped), BufferJSON.reviver) }
