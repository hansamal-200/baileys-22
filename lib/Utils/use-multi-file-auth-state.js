...
const unwrapValue = (obj) => { let c = obj; while (c && typeof c === 'object' && !Array.isArray(c) && 'value' in c && Object.keys(c).length === 1) c = c.value; return c }
const ser = (v) => { if (v == null) return null; const clean = unwrapValue(v); const serialized = JSON.stringify(clean, BufferJSON.replacer); return { data: JSON.parse(serialized), __checksum: computeChecksum(serialized) } }
const deser = (wrapped) => { if (wrapped == null) return null; const candidate = unwrapValue(wrapped); if (candidate && typeof candidate === 'object' && 'data' in candidate && '__checksum' in candidate) return JSON.parse(JSON.stringify(candidate.data), BufferJSON.reviver); return JSON.parse(JSON.stringify(candidate), BufferJSON.reviver) }
