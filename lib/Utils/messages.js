    ...
    if (mediaType === 'sticker') {
        try {
            ...
            uploadData.media = await new Sticker(rawBuf, {
                ...
                quality: message.quality || options.quality || 100,
            }).toBuffer()
...
const getRlottieApi = async () => {
    ...
    const wasmBuf = fs.readFileSync(require.resolve('rlottie/wasm'))
...
const tgsToWebp = async (tgsBuffer, { quality = 80, fps = 30 } = {}) => {
    const lottieJson = JSON.parse(zlib.gunzipSync(tgsBuffer).toString())
    ...
    const tmpdir = fs.mkdtempSync(path.join(os.tmpdir(), 'tgs_'))
    try {
...
export const prepareStickerPackMessage = async (stickerPack, options) => {
    ...
    if (Array.isArray(stickerPack)) {
        ...
        if (keys.length && keys.every(k => !isNaN(k))) {
            stickerPack = { stickers: Object.values(stickerPack) }
        }
    }
    ...
    const processSingleSticker = async (s, i) => {
        try {
            ...
            if (s.isLottie) {
                ...
                const animatedWebp = await sharp(buf, { animated: true }).webp({ quality: 80 }).toBuffer()
                webpBuffer = await new Sticker(animatedWebp, {
                    pack: packName,
                    author: authorName,
    ...
    // Process in parallel batches to balance speed and memory
    for (let i = 0; i < stickers.length; i += BATCH_SIZE) {
        const batch = stickers.slice(i, i + BATCH_SIZE)
        const results = await Promise.all(batch.map((s, j) => processSingleSticker(s, i + j)))
        validStickers.push(...results.filter(Boolean))
        options.logger?.debug(`processed stickers ${i + 1}-${Math.min(i + BATCH_SIZE, stickers.length)} of ${stickers.length}`)
    }
    ...
    const coverWebpBuffer = await new Sticker(coverBuf, {
        pack: packName,
        author: authorName,
