...
export const prepareStickerPackMessage = async (stickerPack, options) => {
    ...
    const stickerPromises = stickers.map(async (s, i) => {
        try {
            const { stream } = await getStream(s.data || s.sticker || (s.buffer?.data ? Buffer.from(s.buffer.data) : s.buffer))
            const buffer = await toBuffer(stream)
            if (!buffer?.length) { skippedStickers.push({ index: i, reason: 'Empty buffer' }); return null }

            const webpBuffer = s.isLottie
                ? await tgsToWebp(buffer, { quality: 80, fps: 30 })
    ...
    const coverWebpBuffer = await new Sticker(coverBuffer, {
        pack: packName,
        author: authorName,
    ...
    const processBatch = async (batch, batchIdx, totalBatches) => {
        ...
        let thumbRes = null
        try {
            const thumbBuf = await sharp(coverBuffer).resize(252, 252).jpeg().toBuffer()
